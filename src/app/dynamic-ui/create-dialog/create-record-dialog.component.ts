      ...
      <div class="dialog-panel font-sans" role="dialog" aria-modal="true" aria-labelledby="create-dialog-title">
        ...
        <header class="dialog-header">
          <div class="header-left">
            <span class="dialog-badge font-mono">CREATE RECORD</span>
            <app-http-badge [method]="activeOperation().method || 'POST'" />
        ...
        <!-- Dialog Subheader / Summary -->
        <div class="dialog-subheader">
          <div class="sub-left">
            @if (activeOperation().summary) {
              <span id="create-dialog-title" class="op-summary">{{ activeOperation().summary }}</span>
            } @else {
              <span id="create-dialog-title" class="op-summary font-mono">{{ activeOperation().method || 'POST' }} {{ activeOperation().path }}</span>
            }
          </div>
        </div>
        ...
        <div class="dialog-body">
          ...
          @if (activeMissingParams().length > 0) {
            <div class="missing-params-section">
              ...
              <p class="missing-desc">
                Parameters required to construct the creation endpoint path:
              </p>

              <div class="params-inputs-list">
                @for (p of activeMissingParams(); track p.name) {
                  <div class="param-field">
                    <label class="param-label font-mono" [for]="'create-param-' + p.name">
                      {{ p.name }}
          ...
          <!-- Request Payload Form Section -->
          <div class="form-section">
            <div class="form-section-header">
              <div class="section-title">
                <mat-icon class="sec-icon">add_circle_outline</mat-icon>
                <span>Request Payload ({{ activeOperation().method || 'POST' }})</span>
              </div>
        ...
        <footer class="dialog-footer">
          ...
          >
            @if (isExecuting()) {
              <app-loading-indicator [inline]="true" size="sm" message="Creating..." />
            } @else {
...
export class CreateRecordDialogComponent implements OnInit, OnChanges {
  ...
  @Output() created = new EventEmitter<ApiExecutionResult>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['initialParams'] || changes['initialValue'] || changes['missingParams']) {
      this.initComponentState();
    }
  }
  ...
  private initComponentState(): void {
    ...
    for (const p of this.activeMissingParams()) {
    ...
    const initVal = this.initialValue ? { ...this.initialValue } : null;
    this.formInitialValue.set(initVal);
    this.formCurrentValue.set(initVal || {});
    this.rawJsonText.set(initVal ? JSON.stringify(initVal, null, 2) : '{}');
  ...
  areAllMissingParamsProvided(): boolean {
    ...
    return this.activeMissingParams().every((p) => {
      const v = vals[p.name];
        ...
        const errorMsg =
          err instanceof Error
            ...
            : typeof errorObj?.['message'] === 'string'
              ? errorObj['message']
              : 'Error executing creation operation';
